function[M]=bmcc_asymmetric_splitting_2_large_scale_MT_2(d)
%%% Asymmetric Operator Splitting for 2 Pool Henkelman model, large scale
% See
...
%%% calculates the magnetization vector M using an
%%% asymmetric operating spliting method
...
A2(1,1)=-1/d.T2(1)*d.relax(1);
A2(2,2)=-1/d.T2(1)*d.relax(1);
...
for n=1:Nu-1 % time loop
    ...
    deltaw=-w+wref*d.dw(2);
    Rrfc=d.gamma.^2*(d.u(n).^2+d.v(n).^2)*d.B1c*d.T2(2)./(1+(deltaw*d.T2(2)).^2);
    
    Mt(6,:)=Mt(6,:).*exp(-Rrfc*d.dt);
