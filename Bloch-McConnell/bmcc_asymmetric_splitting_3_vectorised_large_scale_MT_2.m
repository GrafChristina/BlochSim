function[M]=bmcc_asymmetric_splitting_3_vectorised_large_scale_MT_2(d)
%%% Asymmetric Operator Splitting for 3 Henkelman model, large scale
% See
...
A2(4,4)=-1/d.T2(2)*d.relax(2)-d.k(3);
A2(5,5)=-1/d.T2(2)*d.relax(2)-d.k(3);
A2(6,6)=-1/d.T1(2)*d.relax(2)-d.k(3);
A2(7,7)=1;
A2(8,8)=1;
A2(9,9)=-1/d.T1(3)*d.relax(3)-d.k(5);
...
for n=1:d.Nu-1 % time loop
    ...
    wref=d.gamma*d.B0;
    w=wref*d.xZspec;
    K1=repmat((-w+wref*d.dw(1))*d.dt,1,1)';
    K2=repmat((-w+wref*d.dw(2))*d.dt,1,1)';
    ...
    deltaw=-w+wref*d.dw(3);
    Rrfc=d.gamma.^2*(d.u(n).^2+d.v(n).^2)*d.B1c*d.T2(3)./(1+(deltaw*d.T2(3)).^2);
    
    Mrot(9,:)=Mrot(9,:).*exp(-Rrfc*d.dt);
