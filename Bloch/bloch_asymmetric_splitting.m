function[M]=bloch_asymmetric_splitting(u,v,w,d)
%%% Asymmetric splitting based solver for Bloch's equation with relaxation.
% See
...
Nx=size(xdis,2);                                            %number of spatial points
Nu=size(u,1);                                               %number of time points
M0=d.M0;                                                    %initial magnetization
dt=d.dt;                                                    %time step length


gadt = d.gamma*d.dt;                                        %time step multiplied with gamma
...
for n=1:Nu 
    ...
    %rotation
    Mrot(1,:)=Bd1(:,n)'.*Mn(1,:)+Bd2(:,n)'.*Mn(2,:)+Bd3(:,n)'.*Mn(3,:);
    Mrot(2,:)=Bd4(:,n)'.*Mn(1,:)+Bd5(:,n)'.*Mn(2,:)+Bd6(:,n)'.*Mn(3,:);
    Mrot(3,:)=Bd7(:,n)'.*Mn(1,:)+Bd8(:,n)'.*Mn(2,:)+Bd9(:,n)'.*Mn(3,:);
